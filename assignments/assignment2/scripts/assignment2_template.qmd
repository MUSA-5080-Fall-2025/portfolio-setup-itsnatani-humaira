...
census_api_key(Sys.getenv("28da3bddd17210421250559be63d8c9379ee190c"))

# Load spatial data
pa_counties <- st_read(here("C:/NATAN/PENN/2025 FALL/Public Policy Analytics/portfolio-setup-itsnatani-humaira/assignments/assignment2/scripts/data/Pennsylvania_County_Boundaries.shp"))
hospitals <- st_read(here("C:/NATAN/PENN/2025 FALL/Public Policy Analytics/portfolio-setup-itsnatani-humaira/assignments/assignment2/scripts/data/hospitals.geojson"))
...
hospitals - How many census tracts? 3445 census tracts - What coordinate
reference system is each dataset in? WGS 84 / Pseudo-Mercator
...
pa_tracts_demo <- pa_demo_data %>%
  select(GEOID, NAME, variable, estimate) %>%
  pivot_wider(names_from = variable, values_from = estimate) %>%
  mutate(
    pop65plus = male65_66 + male67_69 + male70_74 + male75_79 +
                male80_84 + male85plus +
                female65_66 + female67_69 + female70_74 +
                female75_79 + female80_84 + female85plus
  )
pa_tracts_demo <- st_transform(pa_tracts_demo, st_crs(pa_counties))

#count missing income data
sum(is.na(pa_tracts_demo$med_income))

#calculate median income across all PA census tracts
median_income_pa <- median(pa_tracts_demo$med_income, na.rm = TRUE)
```

**Questions to answer:** - What year of ACS data are you using? 2022 -
How many tracts have missing income data? 63 tracts - What is the median
income across all PA census tracts? \$70,188
...
pa_vulnerable <- pa_tracts_demo %>%
  mutate(pct65plus = pop65plus / total_pop * 100) %>%
  filter(med_income < 53300, pct65plus >= 20)
...
# Preview
pa_vulnerable %>%
  select(GEOID, NAME, med_income, total_pop, pop65plus, pct65plus)
...
ggplot(df, aes(x = median_income, y = home_value)) +
  geom_point() + geom_smooth(method = "lm")

df <- df %>%
  mutate(
    log_pop = log(total_pop),
    income_to_rent = median_income / median_rent,
    high_income = if_else(median_income > median(median_income, na.rm = TRUE), "High", "Low")
  )
...
train_indices <- sample(1:n, size = 0.7 * 67)
...
cv_model <- train(home_valueE ~ total_popE,
                  data = challenge_data,
                  method = "lm",
