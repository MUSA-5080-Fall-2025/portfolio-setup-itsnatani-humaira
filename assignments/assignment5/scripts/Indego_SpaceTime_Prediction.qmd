...
**Question:** Do you see spatial clustering of errors? What neighborhoods have high errors?
Highest error clusters around the Center City area.

## Temporal Error Patterns

When are we most wrong?

```{r temporal_errors2024}
# MAE by time of day and day type
...
#Add perfect biking weather and poximity to park as potential features
```{r}
# Create Perfect Biking Weather indicator
train2024$PerfectBikeWeather <- ifelse(
  train2024$Temperature >= 60 & train2024$Temperature <= 75 & train2024$Precipitation == 0,
  1, 0
)
...
nn_idx <- st_nearest_feature(train2024, parks)
nearest_parks <- parks[nn_idx, ]
dist_vec <- st_distance(train2024, nearest_parks, by_element = TRUE)
...
model_b <- lm(
  Trip_Count ~ as.factor(hour) + dotw_simple + Temperature + Precipitation +
    lag1Hour + lag3Hours + lag1day + PerfectBikeWeather + nearest_tbl,
  data = train2024
)
```
