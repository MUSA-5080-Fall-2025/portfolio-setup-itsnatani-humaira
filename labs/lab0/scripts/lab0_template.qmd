...
subtitle: "Natan's First Data Analysis"
...
- Problematic names: [`Engine size`, `Fuel type`, and `Year of manufacture` might cause problems because they have spaces on them]
...
**Your answer:** [A tibble just shows the first 10 rows of the data, meanwhile a data frame displays all full rows that the data has]
...
car_data_select<-car_data%>%select(Model, Mileage)

# Select Manufacturer, Price, and Fuel type
car_data_select_2<-car_data%>%select(Manufacturer, Price, `Fuel type`)

# Challenge: Select all columns EXCEPT Engine Size
car_data_select_3<-car_data%>%select(-`Engine size`)
...
**Your answer:** [Because the `Year of manufacture` name has spaces in it]
...
select(car_data, Model, Year, Age, Mileage, mileage_per_year, price_category)
```
...
car_data_toyota <- car_data %>% filter(Manufacturer == "Toyota")

# Find cars with mileage less than 30,000
car_data_mileage <- car_data %>% filter(Mileage < 30000)

# Find luxury cars (from price category) with low mileage
car_data_mileage_luxury <- car_data_mileage %>% filter(price_category == "mid-range")
```

## 4.2 Multiple Conditions

```{r multiple-conditions}
# Find cars that are EITHER BMW OR Porsche
car_data_BMW_or_Porsche <- car_data %>%
  filter(Manufacturer == "BMW" | Manufacturer == "Porsche")

# Find cars with price between $20,000 and $35,000
car_data_20k_35k <- car_data %>%
  filter(Price >= 20000 & Price <= 35000)

# Find diesel cars less than 10 years old
car_data_diesel_less10y <- car_data %>%
  filter(`Fuel type` == "Diesel" & `Year` >= 2015)
```
